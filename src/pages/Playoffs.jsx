...
const Playoffs = () => {
  ...
  const handleOpen = (e, game) => {
    ...
    if (rawData) {
      ...
      // Logic to split flat stats array into two team arrays
      const team1Stats = [];
      const team2Stats = [];
      let isTeam2 = false;
      ...
      // Formatting data structure to match TournamentBoxscoreModal requirements
                  ...
                  >
                    <div className="text-muted small mb-2 fw-bold d-flex justify-content-between align-items-center">
                      ...
                      <div className="d-flex gap-1 align-items-center">
                        {hasOT && <Badge bg="warning" text="dark" className="rounded-pill px-2 py-1" style={{ fontSize: '0.65rem' }}>OT</Badge>}
                        {data && (
                          <Badge 
                            bg="danger" 
                    ...
                    <div className="d-flex justify-content-between fw-bold small align-items-center">
                      <span>{getSeed(game, 0) != null && <span className="text-muted me-1" style={{ fontSize: '0.7rem' }}>({getSeed(game, 0)})</span>}{teams[0]}</span>
                      {visitorScore !== null && <span className="text-danger fs-6">{visitorScore}</span>}
                    </div>
                    <hr className="my-1" style={{ opacity: 0.1 }} />
                    <div className="d-flex justify-content-between fw-bold small align-items-center">
                      <span>{getSeed(game, 1) != null && <span className="text-muted me-1" style={{ fontSize: '0.7rem' }}>({getSeed(game, 1)})</span>}{teams[1]}</span>
                      {homeScore !== null && <span className="text-danger fs-6">{homeScore}</span>}
                    </div>
                  </div>
                );
              })}
...
const highlightImage = { width: '100%', height: 'auto', borderRadius: '4px', border: '4px solid #111', boxShadow: '10px 10px 0px #ff4d4d' };
const imageCaption = { fontWeight: '900', marginTop: '12px', textTransform: 'uppercase', fontSize: '14px', letterSpacing: '1px' };
